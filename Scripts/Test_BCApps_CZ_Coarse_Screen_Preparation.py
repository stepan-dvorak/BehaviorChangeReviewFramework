...
class PreparationTests(unittest.TestCase):
    ...
    def test_retained_worksheet_preserves_prepared_structure(self) -> None:
        ...
        prepared = [SCREEN.prepare_record(item) for item in contexts]
        ...
        for expected, actual in zip(prepared, records):
            self.assertEqual(expected["context_dataset_sha256"], actual["context_dataset_sha256"])
            self.assertEqual(expected["screening_protocol_version"], actual["screening_protocol_version"])
            self.assertEqual(expected["stratum_indicators"], actual["stratum_indicators"])
            self.assertEqual(expected["evidence_availability"]["composition_scope"], actual["evidence_availability"]["composition_scope"])
            self.assertEqual(expected["evidence_availability"]["test_context"], actual["evidence_availability"]["test_context"])
